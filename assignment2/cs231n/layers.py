...
def batchnorm_backward_alt(dout, cache):
    ...
    # alternative
    d_var = ((x - mean) * -0.5 * (var + eps)**-1.5 * dz).sum(axis=0)
    d_mean = (dz * -1 * (var + eps)**-0.5).sum(axis=0) + (d_var * (x - mean) *
                                                          -2 / N).sum(axis=0)
    dx = dz / (var + eps)**0.5 + d_var * 2 * (x - mean) / N + d_mean / N
        ...
        for c in range(C):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    start_j = j * stride
                    ind = np.argmax(
                        x[n, c, start_i:start_i +
                          pool_height, start_j:start_j + pool_width])
                    ind_i, ind_j = np.unravel_index(ind,
                                                    (pool_height, pool_width))
                    dx[n, c, start_i:start_i + pool_height, start_j:start_j +
                       pool_width][ind_i, ind_j] = dout[n, c, i, j]
...
def spatial_batchnorm_forward(x, gamma, beta, bn_param):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    N, C, H, W = x.shape
    nhwc = x.transpose(0, 2, 3, 1).reshape(N * H * W, C)
    out, cache = batchnorm_forward(nhwc, gamma, beta, bn_param)
...
def spatial_batchnorm_backward(dout, cache):
    ...
    N, C, H, W = dout.shape
    nhwc = dout.transpose(0, 2, 3, 1).reshape(N * H * W, C)
    dx, dgamma, dbeta = batchnorm_backward_alt(nhwc, cache)
